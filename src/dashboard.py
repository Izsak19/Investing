...
def build_table(
    step: int, price: float, action: str, reward: float, portfolio: Portfolio, agent: BanditAgent, success_rate: float
) -> Table:
...
def live_dashboard(events: Iterable[tuple[int, float, str, float, Portfolio, BanditAgent, float]]):
    with Live(refresh_per_second=int(1 / config.DASHBOARD_REFRESH), console=console, screen=False) as live:
        for step, price, action, reward, portfolio, agent, success_rate in events:
            live.update(build_table(step, price, action, reward, portfolio, agent, success_rate))
